...
import { 
  // APIs RAG (renombradas para usar como conversaciones)
  createChatbot as createConversation,
  listUserChatbots as listConversations,
  deleteChatbot as deleteConversation,
  uploadDocuments,
...
const CreateConversation: React.FC = () => {
  ...
  // Estados para conversaciones (ahora usando lógica RAG internamente)
  ...
  // ===== LÓGICA CONVERSACIONES (usando APIs RAG internamente) =====
  ...
  const crearConversacion = async () => {
    ...
    try {
      // Usar API de chatbots internamente
      const chatbot = await createConversation({
        title: titulo.trim(),
        description: descripcion.trim() || undefined
      });
      
      // Mapear respuesta de chatbot a formato de conversación
      const mapped: Conversation = {
        id: chatbot.id,
        titulo: chatbot.title,
        description: chatbot.description,
        usuarios: [], // Se gestionan por separado
        archivos: [], // Se cargan después
        fechaCreacion: new Date(chatbot.created_at).toISOString(),
        documents_count: chatbot.documents_count,
        users_count: chatbot.users_count,
  ...
  const eliminarConversacion = async (conversationId: number) => {
    if (!confirm('¿Eliminar esta conversación? Se perderán todos sus documentos y configuración.')) return;
  ...
  const seleccionarConversacion = async (conversation: Conversation) => {
    ...
    try {
      const docs: ConversationDocument[] = await listChatbotDocuments(conversation.id);
  ...
  const subirDocumentos = async (files: FileList | null) => {
    if (!files || !selectedConv) return;
    setUploadingDocs(true);
    
    try {
      const uploadedDocs: ConversationDocument[] = await uploadDocuments(selectedConv.id, files);
      setConversationDocuments(prev => [...uploadedDocs, ...prev]);
  ...
  const procesarDocumentos = async () => {
    ...
    try {
      await processDocuments(selectedConv.id);
      ...
      setTimeout(async () => {
        try {
          const docs: ConversationDocument[] = await listChatbotDocuments(selectedConv.id);
          setConversationDocuments(docs);
        } catch (e) {
  ...
  const eliminarDocumento = async (docId: number) => {
    if (!selectedConv || !confirm('¿Eliminar este documento?')) return;
    
    try {
      await deleteChatbotDocument(selectedConv.id, docId);
  ...
  // ===== EFECTOS =====
  useEffect(() => {
    // Cargar conversaciones (usando API de chatbots internamente)
    listConversations().then((data) => {
      const mapped = data.map((chatbot) => ({
        id: chatbot.id,
        titulo: chatbot.title,
        description: chatbot.description,
        usuarios: [], // Se manejan por separado
        archivos: [], // Se cargan por separado
        fechaCreacion: new Date(chatbot.created_at).toISOString(),
        documents_count: chatbot.documents_count,
        users_count: chatbot.users_count,
        is_active: chatbot.is_active
      ...
      <div className="admin-card">
        ...
        <div className="create-form">
          ...
          <div className="create-actions">
            <button className="small" onClick={() => { setTitulo(''); setDescripcion(''); setUsuarios([]); setArchivos([]); setUsuarioQuery(''); }}>
              Cancelar
            </button>
            <button 
