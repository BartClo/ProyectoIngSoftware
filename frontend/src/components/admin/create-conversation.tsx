...
const CreateConversation: React.FC = () => {
  ...
  const onPickFiles = (files: FileList | null) => {
    ...
    for (const f of Array.from(files)) {
      if (!acceptMime.includes(f.type) && !acceptAttr.split(',').some(ext => f.name.toLowerCase().endsWith(ext.trim()))) {
        continue; // skip no aceptados
      }
  ...
  const isValid = titulo.trim().length > 0 && usuarios.length > 0 && archivos.length > 0;
              ...
              {pageItems.map(c => (
                <tr key={c.id} className="fixed-height">
                  ...
                  <td className="row-actions" style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    <button className="small" title="Eliminar" onClick={() => {
    ...
    <div className="report-modal-overlay" onClick={onClose}>
      <div className="report-modal-content" onClick={e => e.stopPropagation()}>
        ...
        <div className="form-actions" style={{ padding: '0 16px 16px 16px' }}>
          ...
          <button className="submit-button" onClick={() => onSave(files)}>Guardar</button>
        </div>
      </div>
    </div>
  );
